...
const Pptx = (
  typeof PptxDefault === "function" ? PptxDefault : (PptxDefault as unknown as { default: unknown }).default
) as { new (): any };
...
function titleSlide(pptx: any, d: DeckData): void {
  ...
  s.addText(`Period: ${d.period.label}`, {
    x: 0.5,
    y: 4.0,
    w: 12.3,
    h: 0.5,
  ...
  s.addText(`Generated ${d.generatedAt}`, {
    x: 0.5,
    y: 6.6,
    w: 12.3,
    h: 0.4,
...
function kpiSlide(pptx: any, d: DeckData): void {
  ...
  s.addText(
    insights(d).map((t) => ({
      text: t,
      options: { bullet: true, fontSize: 14, color: INK, breakLine: true },
    })),
...
function salesTrendSlide(pptx: any, d: DeckData): void {
  ...
  if (d.byDay.length === 0) {
    s.addText("No sales in this period.", {
      x: 0.5,
      y: 3,
      w: 12.3,
      h: 1,
  ...
  s.addChart(
    ...
    [
      {
        name: "Sales (₹)",
        labels: d.byDay.map((r) => r.day.slice(5)),
        values: d.byDay.map((r) => rupees(r.total)),
      },
    ],
    {
      x: 0.5,
...
function topItemsSlide(pptx: any, d: DeckData): void {
  ...
  if (d.top.length === 0) {
    s.addText("No items sold in this period.", {
      x: 0.5,
      y: 3,
      w: 12.3,
      h: 1,
  ...
  if (d.slabs.length > 0) {
    ...
    s.addChart(
      ...
      [
        {
          name: "CGST",
          labels: d.slabs.map((x) => `${x.rate}%`),
          values: d.slabs.map((x) => rupees(x.cgst)),
        },
        {
          name: "SGST",
          labels: d.slabs.map((x) => `${x.rate}%`),
          values: d.slabs.map((x) => rupees(x.sgst)),
        },
      ],
      {
        x: 0.4,
...
function healthSlide(pptx: any, d: DeckData): void {
  ...
  const khataLines =
    d.outstanding.length === 0
      ...
      : [
          {
            text: `Total owed: ${formatINR(total)}`,
            options: { fontSize: 14, bold: true, color: INK, breakLine: true },
          },
          ...d.outstanding.map((o) => ({
...
function insights(d: DeckData): string[] {
  ...
  out.push(
    `GST collected: ${formatINR(d.summary.gst)} (CGST ${formatINR(d.summary.cgst)} + SGST ${formatINR(d.summary.sgst)})`,
  );
